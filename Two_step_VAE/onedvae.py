...
def load_onedvae():
    ...
    we use 1d vae for generate Lead I -> Leada II
    1d-vae take (1,512,1) and final output is (1,512,1)
    ...
    #  encoder
    encoder_inputs = keras.Input(shape=(8, 512, 1), name='inference_generator_input')
    number_of_filter_encoder = [64, 64, 128, 128, 256, 256]
    number_of_filter_decoder = [256, 256, 128, 128, 64, 64]
    kernel = (1, 16)
    stride = [(1, 2), (1, 2), (1, 2), 1, 1, 1]
    initializer = tf.random_normal_initializer(0., 0.02)

    encoder_inputs = keras.Input(shape=(1, 512, 1), name='data')
    ...
    b_output = tf.keras.layers.Conv2DTranspose(number_of_filter_decoder[5], kernel, strides=(1, 2), padding='same')(b_output)
    ...
    onedvae = keras.models.Model(inputs=[encoder_inputs], outputs=[reconstructions])
    latent_loss = -0.5 * K.sum(
        1 + z_log_var - K.exp(z_log_var) - K.square(z_mean),
        axis=1)
    onedvae.add_loss(K.mean(latent_loss))
    onedvae.add_metric(K.mean(latent_loss), name="kl", aggregation="mean")
    return onedvae

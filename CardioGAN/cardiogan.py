...
def load_generator():
    '''
      ...
      we revised CardioGAN from cardio(https://github.com/pritamqu/ppg2ecg-cardiogan) for adjust our data .
      revised-generator take (16,512,1) data and final output is (16,512,1).
      input and output data contain two row zero padding on the top and bottom
      we use 2d-convolution layer, strides 2, 2, 2, 1, 1, and fixed kernel size (2,16)

      initializer : initializer of each layer, random choise in (mean =0, stdev = 0.02)
      inputs : Input of inference generator
      filter size : filter of encoder in inference generator
      connections : save each output of layers for skip connection
